...
for i = 1:length(do_rates)
    ...
    opt.alpha, opt.batchSize, opt.numEpochs, opt.input_do_rate(1), opt.hidden_do_rate(1), errors_o(opt.numEpochs), opt.dataset));
end
figure;
...
for i = 1:length(do_rates)
    ...
    opt.noiseScale, opt.alpha, opt.batchSize, opt.numEpochs, opt.input_do_rate(1), opt.hidden_do_rate(1), errors_o(opt.numEpochs), opt.dataset));
end
figure;
opt.gaussian = false;
opt.dropout = true;
